...
export class ArticleFormComponent implements OnInit, OnChanges, OnDestroy {
    ...
    const uploadDocs$ = this.docsToCreate.length > 0
      ? of(this.docsToCreate).pipe(
          switchMap(docs => {
            ...
            return this.articleService.uploadDocumentsForArticle(formDataDocs);
          }),
          map(uploadResponse =>
            uploadResponse.map((media, index) => ({
              number: index + 1,
  ...
  private updatedArticle(): void {
    ...
    forkJoin({
      ...
      switchMap(({ images, docs }) => {
        ...
        // Actualizar artículo
        return this.articleService.updateArticle(this.currentArticle!.uuid, articleUpdated);
      })
    ).subscribe({
  ...
  private updateArticleWithoutNewImagesDocs(): void {
    ...
    this.articleService.updateArticle(this.currentArticle.uuid, articleEdited).subscribe({
      next: (edited) => {
        this.isLoading = false;
  ...
  public deleteArticle(): void {
    ...
    this.isLoading = true;
    this.articleService.deleteArticle(this.currentArticle.uuid).subscribe({
      next: () => {
        this.isLoading = false;
